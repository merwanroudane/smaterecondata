...
```bash
sudo systemctl restart smatecondata-backend.service smatecondata-mcp.service  # Restart prod backend
systemctl status smatecondata-backend.service                             # Check status
sudo journalctl -u smatecondata-backend.service -f                        # Tail prod logs
```

`scripts/restart_dev.py` is for DEV ONLY and refuses to run while `smatecondata-backend.service` is active (it would kill production on port 3001). For dev on another machine:
...
- **Verify production** at http://localhost:3001/chat after deploying fixes
...
sudo journalctl -u smatecondata-backend.service -f   # production
...
sudo systemctl restart smatecondata-backend.service smatecondata-mcp.service
curl http://localhost:3001/api/health
```
...
# Production
NODE_ENV=production
ALLOWED_ORIGINS=http://localhost:3001,https://www.localhost:3001
```
