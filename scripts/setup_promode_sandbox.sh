...
#      uid (merwanroudane) entirely — a kernel-namespace bug can't reach merwanroudane's
#      files/processes because the process simply isn't merwanroudane.
#   2. A shared group `promode-share` so the backend (merwanroudane) can still
#      collect output files written by promode (setgid on the output tree).
#   3. A TIGHTLY-scoped sudoers drop-in letting ONLY merwanroudane run ONLY
...
PUBLIC_DIR="${PROMODE_PUBLIC_DIR:-/opt/smatecondata/public_media/promode}"
...
#    group-owned by promode-share and collectable by merwanroudane.
#    NOTE: the per-exec output dir is created by the backend (merwanroudane) under
...
#    merwanroudane (also in the group) can shutil.move them.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 3. Scoped sudoers drop-in. ONLY merwanroudane, ONLY to run unshare AS promode.
