...
EXPECTED_SHA="${1:-}"
REPO_ROOT="/opt/smatecondata"
PROD_HEALTH_URL="http://localhost:3001/api/health"
PROD_QUERY_URL="http://localhost:3001/api/query"
LOG_PATH="/tmp/smatecondata_after_hours_push_and_verify.log"
SMOKE_JSON="/tmp/smatecondata_after_hours_push_and_verify_smoke.json"
...
for i in $(seq 1 30); do
  if /usr/bin/curl -sf --max-time 10 "$PROD_HEALTH_URL" > /tmp/smatecondata_after_hours_prod_health.json; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S %Z %z')] production health OK"
    cat /tmp/smatecondata_after_hours_prod_health.json
    break
  fi
