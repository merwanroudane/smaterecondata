...
def resolve_python() -> Path:
    candidates = [
        ...
        Path("/opt/smatecondata/backend/.venv/bin/python"),
    ]
...
def main() -> int:
    ...
    env = os.environ.copy()
    env["SMATECONDATA_MULTIROUND_BASE_URL"] = str(args.base_url).rstrip("/")
    env["SMATECONDATA_MULTIROUND_REPORT"] = str(report_path)
    env["SMATECONDATA_MULTIROUND_SUITE"] = str(args.suite)
    env["SMATECONDATA_MULTIROUND_MIN_EFFECTIVE_RATE"] = str(args.min_effective_rate)
    env["SMATECONDATA_MULTIROUND_MAX_FAILS"] = str(args.max_fails)
    env["SMATECONDATA_MULTIROUND_REQUEST_TIMEOUT"] = str(args.request_timeout)
    env["SMATECONDATA_MULTIROUND_ROUND_DELAY_SECONDS"] = str(args.round_delay_seconds)
    env["SMATECONDATA_MULTIROUND_BETWEEN_TEST_DELAY_SECONDS"] = str(args.between_test_delay_seconds)
    env["SMATECONDATA_MULTIROUND_MAX_RETRIES"] = str(args.max_retries)
