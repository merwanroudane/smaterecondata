...
SHARED_BACKEND_VENV = Path("/opt/smatecondata/backend/.venv")
...
def start_backend() -> bool:
    ...
    venv_path = resolve_backend_venv()
    use_reload = os.environ.get("SMATECONDATA_DEV_RELOAD", "1").strip().lower() not in {"0", "false", "no", "off"}
    ...
    if restart_backend:
        try:
            unit_state = subprocess.run(
                ["systemctl", "is-active", "smatecondata-backend.service"],
        ...
        if unit_state == "active":
            print_error(
                "smatecondata-backend.service is ACTIVE — this script would kill the "
                "PRODUCTION backend on port 3001."
            )
            print_error(
                "For production restarts use: sudo systemctl restart smatecondata-backend.service "
                "(or scripts/deploy_production.sh)."
            )
            print_error(
                "For local dev on this host, stop the service first: "
                "sudo systemctl stop smatecondata-backend.service"
            )
            return 1
