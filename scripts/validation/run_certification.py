...
def unsupported_direct_surface_reason(row: dict[str, Any], audit: dict[str, Any] | None = None) -> str | None:
    ...
    available through SmatEconData's documented public/runtime provider surface.
...
def runtime_supportability_reason(row: dict[str, Any], resp_json: dict[str, Any]) -> str | None:
    ...
    if provider == 'IMF' and (
        ...
        or 'not yet executable by smatecondata' in evidence and 'imf dataset-family routing' in evidence
    ):
