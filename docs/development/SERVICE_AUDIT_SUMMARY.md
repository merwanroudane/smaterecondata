...
class ConfigurationError(smaterecondataError): pass  # Missing API keys, bad config
class AuthenticationError(smaterecondataError): pass  # Auth failures
class DataProviderError(smaterecondataError): pass   # API failures
class ValidationError(smaterecondataError): pass     # Input validation
```
