...
**Date**: 2025-12-24
**Scope**: `/home/merwanroudane/smaterecondata/backend/services/`
...
class smaterecondataError(Exception): pass
class ConfigurationError(smaterecondataError): pass  # for missing keys, bad config
class AuthenticationError(smaterecondataError): pass  # for auth failures
class DataProviderError(smaterecondataError): pass   # for API failures
class ValidationError(smaterecondataError): pass     # for input validation
```
