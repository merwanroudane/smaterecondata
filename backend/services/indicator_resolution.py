...
async def resolve_indicator_for_fetch(
    ...
    try:
        ...
        # so every select() threads the region identically).
        from .indicator_selector import build_region_selection_kwargs

        _region_kw: Dict[str, Any] = build_region_selection_kwargs(
            getattr(intent, "subnationalRegion", None),
            provider,
            getattr(svc, "statscan_provider", None),
        )
        if metadata_query:
