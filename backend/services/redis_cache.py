"""
Redis caching service for SmatEconData.
...
class RedisCacheService:
    ...
    def _generate_key(self, provider: str, query: str, params: Optional[Dict] = None) -> str:
        ...
        return f"smatecondata:{provider.lower()}:{key_hash}"
    ...
    async def clear_provider(self, provider: str) -> int:
        ...
        """
        pattern = f"smatecondata:{provider.lower()}:*"
        deleted = 0
    ...
    async def clear_all(self) -> int:
        """
        Clear all SmatEconData cache entries from Redis and in-memory fallback cache.
        ...
        if self._connected and self.redis_client:
            try:
                cursor = 0
                while True:
                    cursor, keys = await self.redis_client.scan(cursor, match="smatecondata:*", count=200)
                    if keys:
                        deleted += await self.redis_client.delete(*keys)
                    if cursor == 0:
                        break
                logger.info(f"🧹 Cleared {deleted} Redis cache entries for SmatEconData")
            except Exception as e:
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        if self._connected and self.redis_client:
            try:
                ...
                # Count SmatEconData keys
                smatecondata_keys = 0
                cursor = 0
                while True:
                    cursor, keys = await self.redis_client.scan(cursor, match="smatecondata:*", count=100)
                    smatecondata_keys += len(keys)
                    if cursor == 0:
                        break
                stats["redis_stats"]["smatecondata_keys"] = smatecondata_keys
