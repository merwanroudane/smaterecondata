"""Custom exception hierarchy for SmatEconData.
...
class EconDataMcpError(Exception):
    """Base exception for all SmatEconData errors.

    All custom exceptions should inherit from this class to enable
    catching all SmatEconData-specific errors with a single except block.
...
def get_error_response(error: Exception) -> Dict[str, Any]:
    ...
    # For non-SmatEconData exceptions, create a generic response
    return {
        "error": "InternalError",
        "message": str(error),
        "details": {},
