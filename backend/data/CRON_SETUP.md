...
This guide explains how to set up automatic daily updates for the SmatEconData catalog database.
...
```bash
sudo mkdir -p /var/log/smatecondata
sudo chown $USER:$USER /var/log/smatecondata
```
...
```
======================================================================
SMATECONDATA CATALOG UPDATE - 2025-11-02T02:00:00
======================================================================
...
```cron
# SmatEconData Catalog Update - Daily at 2 AM
0 2 * * * cd /opt/smatecondata && source backend/.venv/bin/activate && python backend/data/update_catalog.py >> /var/log/smatecondata/catalog_update.log 2>&1
```
...
```bash
tail -f /var/log/smatecondata/catalog_update.log
```

View recent updates:

```bash
tail -100 /var/log/smatecondata/catalog_update.log
```
...
```cron
# Daily at 3 AM
0 3 * * * cd /opt/smatecondata && ...

# Every 6 hours
0 */6 * * * cd /opt/smatecondata && ...

# Weekly on Sunday at 2 AM
0 2 * * 0 cd /opt/smatecondata && ...
```
...
Create `/etc/logrotate.d/smatecondata`:

```bash
sudo nano /etc/logrotate.d/smatecondata
```

Add:

```
/var/log/smatecondata/*.log {
    daily
    rotate 30
    compress
    delaycompress
    missingok
    notifempty
    create 644 merwanroudane merwanroudane
}
```
...
```bash
sudo logrotate -f /etc/logrotate.d/smatecondata
```
...
```bash
grep -E "(✅|❌|SUMMARY)" /var/log/smatecondata/catalog_update.log | tail -50
```
...
```cron
0 2 * * * cd /opt/smatecondata && source backend/.venv/bin/activate && python backend/data/update_catalog.py >> /var/log/smatecondata/catalog_update.log 2>&1 || echo "SmatEconData catalog update failed" | mail -s "SmatEconData Update Failed" your@email.com
```
...
```cron
0 2 * * * cd /opt/smatecondata && sudo systemctl stop smatecondata-backend && source backend/.venv/bin/activate && python backend/data/update_catalog.py >> /var/log/smatecondata/catalog_update.log 2>&1 && sudo systemctl start smatecondata-backend
```
