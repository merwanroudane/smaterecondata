...
class BaseProvider(ABC):
    ...
    async def fetch_data(self, **params) -> NormalizedData | list[NormalizedData]:
        """Public, provider-agnostic entry point.

        Every connector implements ``_fetch_data``, but each also exposes its
        own differently-named public method (``fetch_indicator``,
        ``fetch_series``, ``fetch_exchange_rate``, ...). Callers that work
        across providers -- the dataset gateway, contract tests -- need one
        uniform way in, without reaching into a private method.

        Args:
            **params: Provider-specific parameters. Common keys are
                ``indicator``, ``country``/``countries``, ``start_date`` and
                ``end_date``.
