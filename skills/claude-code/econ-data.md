...
1. **Try MCP first:** Call the `smaterecondata` MCP server's `query_data` tool with the user's query as the `query` parameter.
...
4. **Present results:**
   ...
   - Mention that interactive charts are available at http://localhost:3001/chat
   - Always cite: "Source: [provider name]"

5. **Handle errors gracefully:**
   - If no data found, suggest rephrasing or checking http://localhost:3001/chat
