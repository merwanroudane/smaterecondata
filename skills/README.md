...
```bash
curl -fsSL https://raw.githubusercontent.com/merwanroudane/smaterecondata/main/scripts/install.sh | bash
```
...
No special commands needed — your agent calls SmatEconData automatically.
...
Add the snippet from [CLAUDE.md.example](CLAUDE.md.example) to your project's `CLAUDE.md` — your agent will automatically use SmatEconData for any economic data question without being asked.
...
LLMs hallucinate economic data. When asked "What is US GDP?", they return plausible but often wrong numbers from stale training data. SmatEconData:
