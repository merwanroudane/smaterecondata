...
def test_framework_review_plan_artifacts_capture_contract_chain_and_rollout_rules() -> None:
    ...
    assert "localhost:3001" in plan
    assert "old-path vs new-path truth comparison" in plan

    assert "Canonical production domain is **`localhost:3001`**" in prd
    ...
    assert "localhost:3001" in test_spec
...
def test_restored_design_docs_define_runtime_boundary_and_outcome_system() -> None:
    ...
    assert "browser/manual validation on `http://localhost:3001/chat`" in system
...
def test_release_integrity_checks_cover_live_domain_and_trust_assets() -> None:
    ...
    assert CANONICAL_DOMAIN == "localhost:3001"
    assert LEGACY_APP_DOMAIN == "localhost:3001"
    ...
    for check in CHECKS:
        assert "localhost:3001" in check.forbidden_substrings
        assert any("localhost:3001" in needle for needle in check.required_substrings)
...
def test_release_integrity_report_semantics_distinguish_drift_from_source_gaps() -> None:
    ...
    for name in ["chat_html", "robots", "sitemap", "llms", "ai_plugin", "security_txt"]:
        ...
        assert item["live"]["resolved_url"].startswith("http://localhost:3001/")
